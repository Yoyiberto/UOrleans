# Data Fusion for Mobile Robot Localization
...
Using Euler's method for discretization with a time step $\Delta t$, we obtain the discrete-time state equation

$$
X_{k+1} = f(X_k, u_k) =
\begin{bmatrix}
x_k + u_k \cos(\theta_k)\, \Delta t \\
y_k + u_k \sin(\theta_k)\, \Delta t \\
...
Z_k = h(X_k) =
\begin{bmatrix}
\sqrt{(x_k - x_{b_1})^2 + (y_k - y_{b_1})^2} \\
\sqrt{(x_k - x_{b_2})^2 + (y_k - y_{b_2})^2} \\
\sqrt{(x_k - x_{b_3})^2 + (y_k - y_{b_3})^2}
\end{bmatrix}
$$
...
3. **Type T (TDOA):** Measures the difference in distance between the robot and two beacons, $B_i$ and $B_j$.
   ...
   $$
   \begin{aligned}
   z_T = \Delta d_{(i,j)} &= \sqrt{(x - x_i)^2 + (y - y_i)^2} \\
                          &\quad - \sqrt{(x - x_j)^2 + (y - y_j)^2}
   \end{aligned}
   $$
...
4. **State Fusion:** The state estimates and covariances from the local filters are periodically sent to a fusion center. The fusion center uses an algorithm (e.g., Covariance Intersection, simple averaging) to combine these estimates into a single, more accurate global state estimate.
